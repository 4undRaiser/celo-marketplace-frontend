...
const Product = ({ id, setError, setLoading, clear }: any) => {
    ...
    // use the usecontractSend hook to delete a product
  const {writeAsync: removeProduct} = useContractSend("deleteProduct", [Number(id)]);
  // use the usecontractSend hook to toggle the sale of a product
  const {writeAsync: toggleSale} = useContractSend("toggleProductSale", [Number(id)]);
  ...
  const deleteProduct = async () => {
    ...
    try {
      ...
      if (!address && openConnectModal) {
       ...
       await toast.promise(async () => {
        ...
        setLoading("Deleting product")
        const deleteTx = await removeProduct();
        await deleteTx.wait();
      }, {
   ...
   // Define a function to toggle the sale a product
   const toggleProductSale = async () => {
    setLoading("toggling product sale");
    try {
      // If the user is not connected, trigger the wallet connect modal
      if (!address && openConnectModal) {
        openConnectModal();
       ...
       await toast.promise(async () => {
        if (!toggleSale) {
          throw "Failed to toggle product sale"
        }
        setLoading("Toggling product sale")
        const toggleTx = await toggleSale();
        await toggleTx.wait();
      }, {
        pending: "Toggling product sale...",
        success: "Product sale toggled successfully",
        error: "Failed to toggle product sale",
      });
      // If there is an error, display the error message
    } catch (e: any) {
      setError(e?.reason || e?.message || "Something went wrong. Try again.");
      ...
      <p className="group">
        ...
        <div className={"m-5"}>
          <div className={"pt-1"}>
            ...
            <div className={"h-40 overflow-y-hidden scrollbar-hide"}>
              ...
              <p className="mt-4 text-2xl font-bold">{product.forSale ? "For sale" : "Not forsale"}</p>
          ...
          {/* Delete product button that deletes a product from the marketplace */}
        ...
        >
          Delete Product
           ...
           {/* Toggle product sale button that set's a product forsale or not forsale */}
           { address === product.owner && (
           <button
          type="button"
          onClick={() => toggleProductSale()}
          className="inline-block ml-4 px-6 py-2.5 bg-black text-white font-medium text-md leading-tight rounded-2xl shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out"
        >
          {product.forSale ? "Pause Sale" : "Resume Sale"}
        </button>
           )}
          </div>
        </div>
